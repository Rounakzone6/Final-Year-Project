...
const HostelInCity = () => {
  ...
  useEffect(() => {
    const fetchHostel = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${backendUrl}/city/${id}/hostel`);
  ...
  return (
    <div className="max-w-[90%] mx-auto py-5">
      <h2 className="text-2xl font-bold mb-4 capitalize">Hostels in {city}</h2>
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {hostel.length > 0 ? (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-500 truncate mb-4">
                    near {hostel.college?.name || "Campus"}
                  </p>
            ...
            <p>No hostel found in this city.</p>
          )}
        </div>
      )}
    </div>
